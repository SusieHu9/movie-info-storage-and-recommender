...
def read_exist_matrix(user_like = False, co_occurance = True):
    # read user_like.json
    if user_like:
        with open('user_like.json') as json_file:
            user_like =json.load(json_file)
            # [{'31': 2.5}, {'2294': 2.0}]
        return user_like
    # read co_occurance.json\
    if co_occurance:
        with open('co_occurrence.json') as json_file:
...
def ItemBasedFiltering(user_like, co_occurance_matrix, user_id:str, n=10, file = '/user/user_data/history/ratings.csv', train = False):
    if train == True:
        ...
        with open("user_like.json", "w") as outfile:
        ...
        with open("co_occurance.json", "w") as outfile:
